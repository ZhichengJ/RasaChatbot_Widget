...
class action_pregunta_uno(Action):
    ...
    def run (self, dispatcher, tracker, domain):
        buttons = []
        buttons.append({"title": 'Menos de 1 segundo' , "payload": "/pregunta_dos{'respuesta1':'Menos de 1 segundo'}"})
        buttons.append({"title": 'Entre 1 y 5 segundos' , "payload": "/pregunta_dos{'respuesta1':'Entre 1 y 5 segundos'}"})
        buttons.append({"title": 'Mas de 9 segundos' , "payload": "/pregunta_dos{'respuesta1':'Mas de 9 segundos'}"})
        dispatcher.utter_message(text="¿Cuánto dura el sonido?", buttons=buttons)
        #FollowupAction(name='action_listen')        
        return[SlotSet('respuesta1',buttons)]#, FollowupAction(name='action_listen')]
...
class action_pregunta_dos(Action):
    ...
    def run (self, dispatcher, tracker, domain):
        buttons = []
        buttons.append({"title": 'Si' , "payload": "/pregunta_tres{'respuesta2':'Si'}"})
        buttons.append({"title": 'No' , "payload": "/pregunta_tres{'respuesta2':'No'}"})
...
class action_pregunta_tres(Action):
    ...
    def run (self, dispatcher, tracker, domain):
        buttons = []
        buttons.append({"title": 'Si' , "payload": "/post_api{'respuesta3':'Si'}"})
        buttons.append({"title": 'No' , "payload": "/post_api{'respuesta3':'No'}"})
        dispatcher.utter_message(text='¿Hay un sonido inicial con un tono distinto al resto?', buttons=buttons)
